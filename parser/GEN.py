...
class Report():
    ...
    def __init__(self, filename, subject, keywords):
        ...
        self.data = json.load(
            open(os.path.join(os.getcwd(), "report.json"), "r"))

        for member in self.data['coverpage']['title']['teachers']['members']:
            if member["Name"] == "" and member["Designation"] == "" and member["Department"] == "":
                self.data['coverpage']['title']['teachers']['members'].remove(
                    member)
    ...
    def format_vertical(self, members):
        val = []
        for member in members:
            val.append("\n\\textcolor{{blue}}{{\\textbf{{{}}}}} & \\textcolor{{blue}}{{\\hspace{{2.5cm}}\\textbf{{{}}}}}\\\\".format(
    ...
    def formatStudentData(self, members):
        ...
        for member in members:
            if member['Name'] == "" and member['USN'] == "":
                ...
                num -= 1
                text += "\\textbf{{{}}} bearing USN \\textbf{{{}}}".format(
                    member['Name'], member['USN'])
                if num > 0:
                    text += ";  "
                    num -= 1
    ...
    def addTitle(self):
        with open("./static/titlepage.tex", "r") as title:
            with open(self.title, "w+") as titlepage:
                for line in title:
                    line = self.replaceTag(
                        "<TOPIC>", line, self.data['coverpage']['title']['subject']['topic'])
                    line = self.replaceTag(
                        "<TITLE>", line, self.data['coverpage']['title']['subject']['title'])
                    line = self.replaceTag(
                        "<SEMESTER>", line, self.data['coverpage']['title']['subject']['semester'])
                    line = self.replaceTag("<STUDENTS>", line, self.format_vertical(
                        self.data['coverpage']['title']['students']['members']))
                    line = self.replaceTag("<TEACHERS>", line, self.format_horizontal(
                        self.data['coverpage']['title']['teachers']['members']))
                    line = self.replaceTag(
                        "<NUM_TEACHERS>", line, "X " * len(self.data['coverpage']['title']['teachers']['members']))
        ...
        with open("./static/certificate.tex", "r") as certificate:
            with open(self.certificate, "w+") as certificatepage:
                for line in certificate:
                    ...
                    line = self.replaceTag(
                        "<SEMESTER>", line, self.data['coverpage']['title']['subject']['semester'])
                    line = self.replaceTag("<GUIDES>", line, self.format_horizontal(
                        self.data['coverpage']['certificate']['members']))
                    line = self.replaceTag(
                        "<NUM_GUIDES>", line, "X " * len(self.data['coverpage']['certificate']['members']))
                    line = self.replaceTag("<STUDENTS>", line, self.formatStudentData(
                        self.data['coverpage']['title']['students']['members']))
    ...
    def addAck(self):
        with open("./static/ack.tex", "r") as ack:
            with open(self.ack, "w+") as ackpage:
                for line in ack:
                    line = self.replaceTag(
                        "<CONTENT>", line, processText(self.data['coverpage']['acknowledgements']['content']))
                    line = self.replaceTag("<STUDENT>", line, self.formatAck(
                        self.data['coverpage']['title']['students']['members']))
                    ackpage.write(line)
        with open(self.filepath, "a") as report:
            report.write("\n\\include{./ack}")
        ...
        with open("./static/abstract.tex", "r") as abstract:
            with open(self.abstract, "w+") as abstractpage:
                for line in abstract:
                    line = self.replaceTag("<CONTENT>", line, processText(
                        self.data['coverpage']['abstract']['content']))
...
# tlmgr install titlesec, adjustbox, collectbox
